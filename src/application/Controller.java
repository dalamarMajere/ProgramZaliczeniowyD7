...
public class Controller implements Initializable {
	...
	*/
	public void studentAddButtonAction() throws IOException {
		...
		if (checkString(surname, name) && checkNumber(semester, id)
				&& checkDate(birthDate) && checkChoiceBox(studentFacultyChoiceBox, studentCourseChoiceBox)) {
	...
	*/
    ...
    public void subjectAddButtonAction() throws IOException {
    	...
    	if(checkString(name) && checkNumber(id) && checkChoiceBox(subjectCourseChoiceBox,subjectFacultyChoiceBox)){
        	Main.loadSubject(id, name, course, faculty);
        	resetValue(subjectNameLabel, subjectIdLabel);
	...
	*/
    ...
    public boolean checkString(String... input) throws IOException{
    	...
    	for (int i = 0; i < input.length; i++){
    		if (input[i].isEmpty()){
    			Main.showAlertBox("Blad Wprowadzania","Wszystkie pola musza byc uzupelnione!");
    			check = false;
    			break;
    		}
    	}
    ...
    public boolean checkChoiceBox(ChoiceBox... input) throws IOException{
    	...
    	for(int i = 0; i < input.length;i++){
    		if (input[i].getValue() == null){
    			Main.showAlertBox("Blad Wprowadzania","Wszystkie pola wyboru musza byc uzupelnione!");
    			check = false;
    			break;
    		}
    	}
    	...
    	for (int i = 0; i < input.length; i++){
    		if (checkString(input[i])) {
    			try {
        			...
        			check = false;
        			Main.showAlertBox("Blad Wprowadzania","Niektore pola musza byc numerem!");
        			break;
        		}
    		} else {
    			check = false;
    ...
    /**
     * returns array of strings in the format: first letter uppercase, other lowercase
     */
    public String[] formatText(String... text){
    	...
    	for(int i = 0; i < values.length; i++){
    		...
    		values[i] = values[i].substring(0,1).toUpperCase() + 
